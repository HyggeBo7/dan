package com.dan.utils.network.exception;
...
public class HttpException {
    ...
    public static HttpUtils.ResultResponse handleException(Throwable e) {
        if (e instanceof ConnectException) {
            return new HttpUtils.ResultResponse(ERROR.NETWORK_ERROR, e.getMessage(), "连接失败!", null);
        } else if (e instanceof SocketTimeoutException) {
            return new HttpUtils.ResultResponse(ERROR.TIMEOUT_ERROR, e.getMessage(), "连接超时!", null);
        } else if (e instanceof UnknownHostException) {
            return new HttpUtils.ResultResponse(ERROR.UNKNOWNHOST_ERROR, e.getMessage(), "无法解析该域名!", null);
        } else if (e instanceof NullPointerException) {
            return new HttpUtils.ResultResponse(ERROR.NULLPOINTER_EXCEPTION, e.getMessage(), "NullPointerException!", null);
        } else if (e instanceof ClassCastException) {
            return new HttpUtils.ResultResponse(ERROR.CAST_ERROR, e.getMessage(), "类型转换错误!", null);
        }
        return new HttpUtils.ResultResponse(ERROR.UNKNOWN, e.getMessage(), "未知错误!", null);
    }
